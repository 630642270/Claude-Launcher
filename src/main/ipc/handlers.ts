...
import { openHistoryWindow, openSettingsWindow } from "../windows";
...
function notifyTerminalLaunched(result: {
...
function pickGlobalSettings(
  ...
  if (partial.terminalMode !== undefined)
    global.terminalMode = partial.terminalMode;
  if (partial.externalTerminal !== undefined)
    global.externalTerminal = partial.externalTerminal;
  if (partial.claudePath !== undefined) global.claudePath = partial.claudePath;
  if (partial.lastProjectPath !== undefined)
    global.lastProjectPath = partial.lastProjectPath;
  if (partial.minimizeToTray !== undefined)
    global.minimizeToTray = partial.minimizeToTray;
  ...
  if (partial.terminalFontSize !== undefined)
    global.terminalFontSize = partial.terminalFontSize;
  if (partial.terminalScrollback !== undefined)
    global.terminalScrollback = partial.terminalScrollback;
...
export function registerIpcHandlers(
  ...
  ipcMain.handle("config:save", async (_event, partial: Partial<AppConfig>) => {
    ...
    const saved =
      Object.keys(global).length > 0 ? saveGlobalSettings(global) : getConfig();
    broadcastConfigChanged();
    return toConfigView(saved);
  });
  ...
  ipcMain.handle(
    "profile:save",
    async (_event, request: ProfileSaveRequest) => {
      ...
      if (hadNewApiKey && saved.apiKey && saved.baseUrl) {
        try {
          return await refreshModelsAndSave(
            saved.baseUrl,
            saved.apiKey,
            saved.modelsUrl,
          );
  ...
  ipcMain.handle("profile:duplicate", (_event, profileId: string) => {
    const saved = duplicateProfile(profileId);
    broadcastConfigChanged();
    return toConfigView(saved);
  });
  ...
  ipcMain.handle(
    ...
    async (_event, request?: ProviderRequest) => {
  ...
  ipcMain.handle(
    ...
    async (_event, request?: ProviderRequest) => {
      ...
      const model = request?.model?.trim() || getConfig().models.main;
      return benchmarkModel(baseUrl, apiKey, model);
    },
  );
  ...
  ipcMain.handle("terminal:session", () => ({
    active: isEmbeddedSessionActive(),
  }));
  ...
  ipcMain.handle("launch", async (_event, options: LaunchOptions) => {
    ...
    if (!detected.found && claudePath === "claude") {
      throw new Error(
        "未找到 claude 命令，请先运行: npm install -g @anthropic-ai/claude-code",
      );
    }
